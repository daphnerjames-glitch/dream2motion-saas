/**
 * Granny YouTube Automation - Apps Script
 * Reads Granny YouTube sheet and sends to Cloud Function
 */

const SECURITY_TOKEN = "closetrebel2026";
const DREAM2MOTION_URL = "https://dream2motion-saas.vercel.app/api/granny/process";

function runGrannyAutomation() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheets()[0]; // First sheet
  const timestamp = new Date().toISOString();

  Logger.log("=== Granny YouTube Automation started at " + timestamp);
  ...
  for (let i = 1; i < values.length; i++) {
    ...
    try {
      const response = UrlFetchApp.fetch(DREAM2MOTION_URL, options);
  ...
  Logger.log("=== Granny automation cycle complete ===");
}

function testGranny() {
  Logger.log("Testing Granny YouTube automation...");
  runGrannyAutomation();
}
