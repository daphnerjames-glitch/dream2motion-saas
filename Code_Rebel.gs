/**
 * Closet Rebel YouTube Automation - Apps Script
 * Reads Closet Rebel YouTube sheet and sends to Cloud Function
 */
...
function runRebelAutomation() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheets()[0]; // First sheet
  const timestamp = new Date().toISOString();

  Logger.log("=== Closet Rebel YouTube Automation started at " + timestamp);
  ...
  Logger.log("=== Closet Rebel automation cycle complete ===");
}

function testRebel() {
  Logger.log("Testing Closet Rebel YouTube automation...");
  runRebelAutomation();
}
